...
public class Invader {
    ...
    public Invader(Context context, int row, int column, int screenX, int screenY) {
        ...
        int padding = screenX / 30;

        x = column * (length + padding);
        y = row * (length + padding / 4);

        // Initialize the bitmap
        bitmap1 = BitmapFactory.decodeResource(context.getResources(), R.drawable.invader1_dark);
        bitmap2 = BitmapFactory.decodeResource(context.getResources(), R.drawable.invader2_dark);
